...
#define MAXPIX		(sizeof(line_buffer)/sizeof(uint32_t))

static void skip(int len)
{
	int n;
	while (len > 0 && (n = read(G.vnc_fd, line_buffer, 
				    MIN(len, sizeof(line_buffer)))) > 0)
		len -= n;
}
...
{
	const char *reset = "\x1b[?25h" "\x1b[2J\x1b[H";
	fb_free();
...
static void term_setup(void)
{
	...
	const char *init = "\x1b[?25l" "\x1b[2J\x1b[H" "** fbvnc **";
...
int fbvnc_main(int argc, char **argv)
{
	...
	vnc_init();
	G.rat_fd = open("/dev/input/mice", O_RDONLY);
	term_setup();
	atexit(cleanup);
